function graphtogml(fname, adjacency, names, CHex, positions)
...
for ii = 1:length(adjacency)
% fprintf(fid,'%s\n',['<node id="n' num2str(ii) '">' '<data key="label">' names{ii} '</data></node>']);   

    
 fprintf(fid,'%s\n',' node [');
 fprintf(fid,'%s\n',['  id ' num2str(ii)]);
 fprintf(fid,'%s\n',['  label "' names{ii} '"']);
 fprintf(fid,'%s\n',['  value ' num2str(adjacency(ii,ii))]);
 fprintf(fid,'%s\n','  graphics [');
  fprintf(fid,'%s\n',['   fill ' '"#' CHex(ii,:) '"']);
 %fprintf(fid,'%s\n',['   y ' num2str(positions(ii,2))]);
 fprintf(fid,'%s\n',['   w ' num2str(adjacency(ii,ii))]);
 fprintf(fid,'%s\n','   type "circle"');
 fprintf(fid,'%s\n','  ]');
 fprintf(fid,'%s\n','  LabelGraphics [');
 fprintf(fid,'%s\n','   type "text"');
...
for ii=find(adjacency(:))'
 ...
 if ii ~= tt
% fprintf(fid,'%s\n',['<edge id="e' num2str(ii) '" source="n' num2str(tt) '" target="n' num2str(ss) '" weight="' num2str(adjacency(ss,tt)) '"/>']);

 
 fprintf(fid,'%s\n',' edge [');
 fprintf(fid,'%s\n',['  label "' num2str(adjacency(ss,tt)) '"']);
 fprintf(fid,'%s\n',['  source ' num2str(tt)]);
 fprintf(fid,'%s\n',['  target ' num2str(ss)]);
 fprintf(fid,'%s\n',['  value ' num2str(adjacency(ss,tt))]);
 %fprintf(fid,'%s\n','  LabelGraphics [');
% fprintf(fid,'%s\n','   type "text"');
% fprintf(fid,'%s\n','  ]');
 fprintf(fid,'%s\n',' ]');
